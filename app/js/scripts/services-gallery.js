let viewport = identifyViewport();

window.addEventListener('resize', onResizingDisplayItems);

function identifyViewport() {
  let currentViewport;

  if (window.matchMedia("(min-width: 992px)").matches) currentViewport = 'desktop';
  if (window.matchMedia("(max-width: 992px)").matches) currentViewport = 'small';
  if (window.matchMedia("(max-width: 768px)").matches) currentViewport = 'mobile';

  return currentViewport;
}

function onResizingDisplayItems() {
  let newViewport = identifyViewport();
...
servicesGridContainer.transitionDuration = 800;
...
Object.defineProperties(servicesGridContainer, {
  'countOfVisibleItems': {
    get: function () {
      ...
      if (viewport === 'mobile') count = 1;
      if (viewport === 'small') count = 2;
      if (viewport === 'desktop') count = 3;
